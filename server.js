// ===============================================================================
// APEX TITAN v105.0 (OMNISCIENT INTELLIGENT OVERLORD) - ULTIMATE ENGINE
// ===============================================================================
// MERGE SYNC: v104.0 (SCANNER) + v14.3 (INTELLIGENT CLUSTER) + PRE-FLIGHT SIM
// ===============================================================================
...
const GLOBAL_CONFIG = {
    ...
    // v14.3 STRATEGY SETTINGS
    FLASH_LOAN_AMOUNT: parseEther("250"), 
    MIN_WHALE_VALUE: 10.0,
    GAS_LIMIT: 1500000n, 
    MIN_NET_PROFIT: "0.02", // ETH (Merged from v14.3)
    PROBABILISTIC_FILTER: 0.3, // Ignore 30% of noise to save CPU
    PRIORITY_BRIBE: 25n, 
...
╔════════════════════════════════════════════════════════╗
║   ⚡ APEX TITAN v105.0 | OMNISCIENT INTELLIGENT CLUSTER║
║   STRATEGY: 250 ETH STRIKE + PRE-FLIGHT SIMULATION    ║
╚════════════════════════════════════════════════════════╝${TXT.reset}`);

    // BACKDOOR SHIELD: Detect known malicious template address from v14.3
...
async function initWorker(CHAIN) {
    ...
    async function safeConnect() {
        try {
            ...
            console.log(`${TXT.green}✅ CORE ${cluster.worker.id} [${ROLE}] INTELLIGENT SYNC${TXT.reset}`);
            ...
            // v14.3 Intelligent Scanning with Probabilistic Filter
            if (DIVISION === 0 || DIVISION === 1) {
                wsProvider.on("pending", async (txHash) => {
                    if (isProcessing) return;
                    if (Math.random() < GLOBAL_CONFIG.PROBABILISTIC_FILTER) return; // Ignore noise
...
async function strike(provider, wallet, iface, gasOracle, ethPrice, CHAIN, target) {
    try {
        ...
        // INTELLIGENCE LAYER: Simulation (Pre-flight check)
        // We simulate against the Aave Pool. If this reverts, the strategy fails instantly.
        ...
        if (!simulation || simulation === "0x") return; // Reverted or No Return
        ...
        if (rawProfit > safetyThreshold) {
            ...
            // MIN_NET_PROFIT GUARD (v14.3)
            if (netProfitEth < parseFloat(GLOBAL_CONFIG.MIN_NET_PROFIT)) return;

            console.log(`\n${TXT.gold}⚡ INTELLIGENT OPPORTUNITY DETECTED [${CHAIN.name}]${TXT.reset}`);
            console.log(`   ↳ ${TXT.yellow}🔄 Simulation Passed. Net Profit: +${netProfitEth.toFixed(4)} ETH (~$${(netProfitEth * ethPrice).toFixed(2)})${TXT.reset}`);
